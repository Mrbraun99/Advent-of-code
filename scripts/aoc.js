...
class AocInput {
    static GRID(input) {
        ...
        return { "grid": grid, "size": { x: grid[0].length, y: grid.length } };
    }

    static HORIZONTAL_CHUNKS(input) {
        const result = [];
        let current = [];
    ...
    static VERTICAL_LINES(input, delimeter) {
        const result = [];

        const rows = input.map((line, index) => {
            return line.split(Array.isArray(delimeter) ? delimeter[index] : delimeter);
        });

        for (let i = 0; i < rows[0].length; i++) {
            result.push(rows.map(row => row[i]));
        }
